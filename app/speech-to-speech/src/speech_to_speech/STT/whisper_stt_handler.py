...
class WhisperSTTHandler(BaseSTTHandler):
    ...
    def process(self, vad_audio: STTIn) -> Iterator[STTOut]:
        ...
        if self.start_language not in (None, "auto"):
            language_code = self.last_language or self.start_language
        else:
            language_code = None
            if pred_ids.ndim >= 2 and pred_ids.shape[1] > 1:
                token = self.processor.tokenizer.decode(pred_ids[0, 1])
                if token.startswith("<|") and token.endswith("|>"):
                    language_code = token[2:-2]

            if language_code not in SUPPORTED_LANGUAGES:
                logger.warning("Whisper detected unsupported or missing language token: %s", language_code)
                if self.last_language is not None:
        ...
        if self.start_language == "auto" and language_code:
            language_code += "-auto"
