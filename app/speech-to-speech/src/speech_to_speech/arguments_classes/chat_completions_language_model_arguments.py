...
class ChatCompletionsLanguageModelHandlerArguments(ResponsesApiLanguageModelHandlerArguments):
    ...
    responses_api_gen_max_tokens: Optional[int] = field(
        default=None,
        metadata={
            "help": "Maximum number of completion tokens requested from the Chat Completions backend. "
            "Default is None."
        },
    )
    responses_api_max_output_characters: Optional[int] = field(
        default=None,
        metadata={
            "help": "Hard character limit applied to assistant text before it reaches TTS and conversation history. "
            "Default is None."
        },
    )
