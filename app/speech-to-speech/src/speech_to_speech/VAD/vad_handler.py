...
class VADHandler(BaseHandler[VADIn, VADOut]):
    ...
    def setup(
        ...
        vad_repo_path = Path(vad_model_repo)
        vad_source = "local" if vad_repo_path.exists() else "github"
        vad_repo = str(vad_repo_path.resolve()) if vad_source == "local" else vad_model_repo
