...
class ChatCompletionsApiModelHandler(BaseOpenAICompatibleHandler):
    ...
    def _request(self, api_input: list[dict[str, Any]], optional_kwargs: dict[str, Any]) -> Any:
        create_kwargs: dict[str, Any] = {
            key: value for key, value in self.gen_kwargs.items() if value is not None
        }
        create_kwargs.update(optional_kwargs)
        if self.stream:
    ...
    # 句末标点。中英文都列上，省略号只认结尾那一个点就够了。
    _SENTENCE_ENDINGS = "。！？!?…；;"

    def _limit_assistant_text(self, text: str, accepted_characters: int = 0) -> str:
        """截断到字数上限，但尽量落在句末。

        原来是直接 text[:remaining]，正好卡在词中间就会出现"说让我"这种
        半句话戛然而止。上限是防止一轮说太久（要过 TTS 和口型渲染），
        本来就不精确，那就宁可少说半句，也别断在句子中间。
        """
        if self.max_output_characters <= 0:
            return text
        remaining = max(0, self.max_output_characters - accepted_characters)
        if remaining >= len(text):
            return text
        clipped = text[:remaining]
        # 往回找最后一个句末标点。找不到就只能硬切——那说明这一整段
        # 里一个句号都没有，硬切和不切一样难看，不如保住字数上限。
        cut = max((clipped.rfind(ch) for ch in self._SENTENCE_ENDINGS), default=-1)
        return clipped[: cut + 1] if cut >= 0 else clipped
    ...
    def _iter_stream_events(self, api_response: Stream[ChatCompletionChunk]) -> Iterator[ProviderEvent]:
        ...
        for chunk in api_response:
            ...
            provider_text = delta.content or getattr(delta, "refusal", None)
            if provider_text:
                text_piece = self._limit_assistant_text(provider_text, len(raw_text))
                output_was_truncated = output_was_truncated or len(text_piece) < len(provider_text)
                if text_piece:
                    raw_text += text_piece
        ...
        if output_was_truncated:
            logger.info("Assistant output truncated to %d characters", self.max_output_characters)
    ...
    def _iter_response_events(self, api_response: Any) -> Iterator[ProviderEvent]:
        ...
        provider_content = message.content or getattr(message, "refusal", None)
        if provider_content:
            raw_content = self._limit_assistant_text(provider_content)
            if len(raw_content) < len(provider_content):
                logger.info("Assistant output truncated to %d characters", self.max_output_characters)
            if raw_content:
