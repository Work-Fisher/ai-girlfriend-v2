...
# Caching substantially reduces repeated compilation time.
...
def _build_realtime_pipeline_unit(
    *,
    ...
    if module_kwargs.llm_backend in ("responses-api", "chat-completions"):
        realtime_lm_vars = vars(responses_api_kw)
    else:
        realtime_lm_vars = vars(lm_kw)
    chat_size = realtime_lm_vars.get("chat_size", 10)
    ...
    service = RealtimeService(
        ...
        initial_instructions=realtime_lm_vars.get("init_chat_prompt"),
    )
...
def get_tts_handler(
    ...
    if module_kwargs.tts == "chatTTS":
        ...
        _speed = _os.getenv("OMNIVOICE_SPEED", "").strip()
        _qwen3_vars = vars(qwen3_tts_handler_kwargs)
        return OmniVoiceTTSHandler(
            stop_event,
            queue_in=lm_response_queue,
            queue_out=send_audio_chunks_queue,
            setup_args=(should_listen,),
            setup_kwargs={
                "base_url": _os.getenv("OMNIVOICE_URL", "http://127.0.0.1:8791"),
                "voice": _os.getenv("OMNIVOICE_VOICE", "").strip() or None,
                "speed": float(_speed) if _speed else None,
                # cancel_scope / speculative_turns 是上游注入到各 TTS kwargs 里的
                # （见 build_pipeline 里那个 for kw in (...) 循环），本函数签名里没有，
                # 所以从 qwen3 的那份取——它一定被注入过，且内容是同一个对象。
                "cancel_scope": _qwen3_vars.get("cancel_scope"),
                "speculative_turns": _qwen3_vars.get("speculative_turns"),
            },
        )
